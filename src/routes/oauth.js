...
/**
 * Issue an access token (JWT) + a refresh token for the given client.
 * Stores the hashed refresh token in the DB and returns the plain-text pair.
 */
function issueTokenPair(client) {
  const accessToken = jwt.sign(
    { sub: client.user_id, type: 'oauth', clientId: client.client_id, clientDbId: client.id },
    process.env.JWT_SECRET,
    { expiresIn: ACCESS_TOKEN_TTL }
  ...
  db.prepare(`
    INSERT INTO oauth_refresh_tokens (client_db_id, user_id, token_hash, expires_at)
    VALUES (?, ?, ?, ?)
  `).run(client.id, client.user_id, refreshTokenHash, expiresAt);
...
/**
 * Extract and validate client credentials from either Basic Auth header
 * or form body fields. Returns { clientId, clientSecret } or null.
 */
function extractClientCredentials(req) {
  let clientId     = req.body?.client_id;
  let clientSecret = req.body?.client_secret;
...
router.post('/token', (req, res) => {
  ...
  if (grantType === 'client_credentials') {
    ...
    const client = db.prepare('SELECT * FROM oauth_clients WHERE client_id = ?').get(String(clientId));
    ...
    const { accessToken, refreshToken } = issueTokenPair(client);
  ...
  if (grantType === 'refresh_token') {
    ...
    if (row.expires_at < now) {
      db.prepare('UPDATE oauth_refresh_tokens SET revoked = 1 WHERE id = ?').run(row.id);
      return res.status(401).json({
        error:             'invalid_grant',
        error_description: 'Refresh token has expired. Please re-authenticate with client_credentials.'
      });
    ...
    const client = db.prepare('SELECT * FROM oauth_clients WHERE id = ?').get(row.client_db_id);
    if (!client) {
      return res.status(401).json({
        error:             'invalid_client',
        error_description: 'OAuth client no longer exists'
    ...
    // Rotate: revoke the current refresh token and issue a new pair atomically
    const rotateAndIssue = db.transaction(() => {
      db.prepare(
        'UPDATE oauth_refresh_tokens SET revoked = 1, reuse_count = reuse_count + 1 WHERE id = ?'
      ).run(row.id);
    ...
    const { accessToken, refreshToken } = rotateAndIssue();
...
/* ── POST /oauth/clients ────────────────────────────────────────────────────── */
router.post('/clients', sessionAuth, (req, res) => {
  const { name } = req.body || {};
  if (!name || typeof name !== 'string' || !name.trim()) {
    return res.status(400).json({ error: 'name is required' });
  }
  ...
  try {
    const result = db.prepare(
      'INSERT INTO oauth_clients (user_id, name, client_id, client_secret_hash) VALUES (?, ?, ?, ?)'
    ).run(req.user.userId, name.trim(), clientId, secretHash);
...
/* ── GET /oauth/clients ─────────────────────────────────────────────────────── */
router.get('/clients', sessionAuth, (req, res) => {
...
/* ── DELETE /oauth/clients/:id ──────────────────────────────────────────────── */
router.delete('/clients/:id', sessionAuth, (req, res) => {
  const id = parseInt(req.params.id, 10);
  if (!Number.isInteger(id)) {
    return res.status(400).json({ error: 'Invalid client id' });
  }
...
/* ── POST /oauth/token  (OAuth 2.0 Client Credentials Grant — RFC 6749) ─────── */
router.post('/token', (req, res) => {
  const body = req.body || {};

  // Credentials can come from the request body OR HTTP Basic Auth header
  ...
  // Constant-time compare even when client not found (prevents timing attacks)
  const hash  = client ? client.client_secret_hash : '$2a$12$invalidhashfortimingnormalization';
  const match = bcrypt.compareSync(String(clientSecret), hash);
